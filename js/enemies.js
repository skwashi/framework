...
function Targeter(grid, x, y, width, height, color, speed, target) {
  ...
  var dir = new Vector(target.getCenter().x - this.getCenter().x,
...
Homer.prototype.move = function (dt) {
  var dir = new Vector(this.target.getCenter().x - this.getCenter().x, this.target.getCenter().y - this.getCenter().y);
  this.angle = Math.atan2(dir.y, dir.x);
  dir.normalize();
